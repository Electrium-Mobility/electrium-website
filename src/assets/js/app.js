...
try {
	...
	function applyTheme() {
		...
		console.log("Applied Theme " + htmlTag.className);
	}

	//Call changeTheme() when something is clicked
	...
	// Function to set the theme state cookie
	function setThemeCookie() {
		console.log("Calling setThemeCookie");
		const expiryDate = new Date();
		expiryDate.setFullYear(expiryDate.getFullYear() + 1); // Expiry set to 1 year from now
	...
	// Function to check if the theme state cookie exists
	function checkThemeCookie() {
		console.log("Calling checkThemeCookie");
		const themeCookie = document.cookie
			.split(";")
			.find((cookie) => cookie.trim().startsWith("theme="));
		if (themeCookie) {
			const cookieValue = themeCookie.split("=")[1];
			isDarkTheme = cookieValue === "dark";
		}
	}
	// Call the function when the page loads
	checkThemeCookie();
	// Call the applyTheme function to apply the initial theme state
	applyTheme();
